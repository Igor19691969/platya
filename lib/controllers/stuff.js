...
exports.list= function(req, res) {
    ...
    if (req.query && req.query.searchStr){
        ...
        } else if (req.query.sale=='blackMini'){
            options.criteria={$and:[{tags: '53c668fa4d0915bf5c09235a'},{tags:'5364db8d707981801b256498'}]}
        } else if (req.query.sale=='ajur'){
            options.criteria={'tags': '5402d59661d370cb046e5ef9'}
        }
        ...
        Category.findById(req.params.category,function (err, result) {
            ...
            if (!result.section){ // раздел
                ...
                Category.find({section:req.params.category},function (err, categories) {
                    ...
                    options.criteria={ category:  {$in: categoryArr}};
                    if (filterTags){
                        options.criteria={ $and: [{ category:  {$in: categoryArr}},{tags:  {$in: filterTags}}]};
                    }
                    Stuff.list(options, function(err, stuffs) {
                        if (err)  return res.json(err);
                        if (page==0){
        ...
        if (filterTags){
            if (options.criteria.$and) {
                options.criteria.$and.push({tags:  {$in: filterTags}});
            } else {
                options.criteria={ $and: [query,{tags:  {$in: filterTags}}]};
            }

        }

        //console.log(options.criteria.$and[1].tags);
        Stuff.list(options, function(err, stuffs) {
            if (err)  return res.json(err);
            if (page==0){
                Stuff.count(options.criteria).exec(function (err, count) {
